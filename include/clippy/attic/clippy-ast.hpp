...
struct Value : Expr {
  ...
  explicit Value(T t) : val(std::move(t)) {}
...
CXX_NORETURN
void typeError() { throw std::runtime_error("typing error"); }
...
any_expr translateNode_internal(JsonExpr& n, VarMap& varmap) {
  static const DispatchTable dt = {
      {"==", &mkOperator<Eq>},         {"===", &mkOperator<StrictEq>},
      {"!=", &mkOperator<Neq>},        {"!==", &mkOperator<StrictNeq>},
      {"if", &mkOperator<If>},         {"!", &mkOperator<Not>},
      {"!!", &mkOperator<NotNot>},     {"or", &mkOperator<Or>},
      {"and", &mkOperator<And>},       {">", &mkOperator<Greater>},
      {">=", &mkOperator<Geq>},        {"<", &mkOperator<Less>},
      {"<=", &mkOperator<Leq>},        {"max", &mkOperator<Max>},
      {"min", &mkOperator<Min>},       {"+", &mkOperator<Add>},
      {"-", &mkOperator<Sub>},         {"*", &mkOperator<Mul>},
      {"/", &mkOperator<Div>},         {"%", &mkOperator<Mod>},
      {"map", &mkOperator<Map>},       {"reduce", &mkOperator<Reduce>},
      {"filter", &mkOperator<Filter>}, {"all", &mkOperator<All>},
      {"none", &mkOperator<None>},     {"some", &mkOperator<Some>},
      {"merge", &mkOperator<Merge>},   {"in", &mkOperator<In>},
      {"cat", &mkOperator<Cat>},       {"log", &mkOperator<Log>},
      {"var", &mkOperator<Var>}};
...
std::tuple<any_expr, std::vector<json::string>, bool> translateNode(
    JsonExpr& n) {
  VarMap varmap;
  any_expr node = translateNode_internal(n, varmap);
  bool hasComputedVariables = varmap.hasComputedVariables();

  return std::make_tuple(std::move(node), varmap.toVector(),
...
using any_expr =
    std::unique_ptr<Expr>;  // could be value if we have a type for that
...
std::int64_t toInt(double d) { return d; }

std::int64_t toInt(bool b) { return b; }

std::int64_t toInt(std::uint64_t v) { return v; }

std::uint64_t toUint(const json::string& str) {
  return std::stoull(std::string{str.c_str()});
}

std::uint64_t toUint(double d) { return d; }

std::uint64_t toUint(std::int64_t v) { return v; }

std::uint64_t toUint(bool b) { return b; }

double toDouble(const json::string& str) {
  return std::stod(std::string{str.c_str()});
}

double toDouble(std::int64_t val) { return val; }

double toDouble(std::uint64_t val) { return val; }
...
json::string toString(bool b) { return json::string{b ? "true" : "false"}; }

json::string toString(std::nullptr_t) { return json::string{"null"}; }
...
bool toBool(any_expr& el) { return toBool(*el); }
...
any_expr convert(any_expr val, const IntegerArithmeticOperator&) {
  struct IntegerArithmeticConverter : FwdVisitor {
    ...
    // need to convert values
    void visit(StringVal& el) final { res = toany_expr(toInt(el.value())); }

    void visit(BoolVal& el) final { res = toany_expr(toInt(el.value())); }

    void visit(DoubleVal& el) final { res = toany_expr(toInt(el.value())); }
...
any_expr convert(any_expr val, const StringOperator&) {
  struct StringConverter : FwdVisitor {
    ...
    // need to convert values
    void visit(BoolVal& el) final { res = toany_expr(toString(el.value())); }

    void visit(IntVal& el) final { res = toany_expr(toString(el.value())); }

    void visit(UintVal& el) final { res = toany_expr(toString(el.value())); }

    void visit(DoubleVal& el) final { res = toany_expr(toString(el.value())); }

    void visit(NullVal& el) final { res = toany_expr(toString(el.value())); }
...
struct BinaryOperatorVisitor_ : FwdVisitor {
  ...
  void visit(IntVal& n) final {
    if constexpr (BinaryOperator::definedForInteger) return calc(&n.value());
...
struct Calc<Not> {
  ...
  result_type operator()(Expr& val) const { return !toBool(val); }
};
...
struct Calculator : FwdVisitor {
  using VarAccess = std::function<any_expr(const json::string&, int)>;
...
void Calculator::visit(Var& n) {
  ...
  any_expr elm = convert(eval(n.operand(0)), StringOperator{});
...
any_expr calculate(any_expr& exp) {
  return calculate(exp,
                   [](const json::string&, int) -> any_expr { unsupported(); });
}
