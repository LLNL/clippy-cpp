...
ColumnID findColumn(const std::vector<std::string>& colnames,
                    boost::string_view colname) {
...
T extractValue(const boost::json::value& val, T&& tag) {
  if (const boost::json::string* s = val.if_string())
    return jsonlogic::toConcreteValue(*s, tag);

  if (const std::int64_t* i = val.if_int64())
    return jsonlogic::toConcreteValue(*i, tag);

  if (const std::uint64_t* u = val.if_uint64())
    return jsonlogic::toConcreteValue(*u, tag);

  if (const double* d = val.if_double())
    return jsonlogic::toConcreteValue(*d, tag);

  if (val.is_null()) return jsonlogic::toConcreteValue(std::nullptr_t{}, tag);

  throw std::logic_error("unsupported json value conversion.");
}

template <class T>
T extractValue(jsonlogic::any_expr&& val, T&& tag) {
  return jsonlogic::unpack_value<T>(std::move(val));
}

void storeAtColumn(DataFrame& frame, const ColumnVariant& coldesc,
                   dataframe_variant_t& cell, const boost::json::value& val) {
  const std::string& celltype = coldesc.type_name();

  if (celltype == string_type_str)
    cell =
        frame.persistent_string_std(extractValue(val, boost::json::string{}));
...
auto toJson(const dataframe_variant_t& el) -> boost::json::value {
  ...
  if (const int_t* i = std::get_if<int_t>(&el)) return boost::json::value{*i};

  if (const real_t* r = std::get_if<real_t>(&el)) return boost::json::value{*r};

  if (const uint_t* u = std::get_if<uint_t>(&el)) return boost::json::value{*u};

  CXX_UNLIKELY;
  return boost::json::value{};
}

void injectValue(boost::json::object& o, const std::string& colname,
                 const dataframe_variant_t& el) {
...
inline void setCellValue(DataFrame& frame, const ColumnVariant& coldesc,
                         std::int64_t row, jsonlogic::any_expr&& val) {
  ColumnVariant::pointer_variant_t ptr = coldesc.at_variant(row);

  if (string_t** s = std::get_if<string_t*>(&ptr))
    **s = frame.persistent_string_std(
        extractValue(std::move(val), boost::json::string{}));
...
void importJson(DataFrame& frame, const boost::json::value& val,
                bool useOtherColumn = true) {
  ...
  for (auto& el : obj) {
    if (ColumnID colid = findColumn(allColumnNames, el.key())) {
      ...
      throw std::logic_error(
          "automatic sparse column creation not yet supported");
  }
...
auto exportJson(const DataFrame& frame, const std::vector<std::string>& sel,
                std::size_t rownum) -> boost::json::object {
  boost::json::object res;
  std::vector<int> colsel =
      sel.size() ? frame.get_index_list_std(sel) : allColumns(frame);
